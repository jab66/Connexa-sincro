import datetime
import uuid
from fnd_barcode2 import BarcodeFunc
import pandas as pd
...
def sincronizar_datos():
    ...
    pd.set_option('display.width', None)  # Deja que pandas ajuste el ancho de la pantalla automáticamente


    barcode_func = BarcodeFunc()

    # obtener el dataframe de los productos de Connexa (solo los activos)
    print("creanado df_productos_connexa")
    df_productos_connexa = barcode_func.create_df_fnd_product() 
    print(len(df_productos_connexa))

    # obtener el dataframe de los productos de Valkimia
    print("creando df_productos_valkimia")
    df_productos_valkimia = barcode_func.create_df_articulos_valkimia() 
    print(len(df_productos_valkimia))

    df_all = pd.DataFrame()

    print("se recorre el dataframe df_productos_connexa")
    ...
    #
    if (len(df_all)>0):
        barcode_func.imprimir_dataframe(nombre_archivo="df_all", dataframe=df_all)
        ...
        print(f"df_articulos_no_encontrados_en_valkimia = {df_articulos_no_encontrados_en_valkimia.shape}")

        if (len(df_articulos_no_encontrados_en_valkimia)>0):
            barcode_func.imprimir_dataframe(nombre_archivo="articulos_no_encontrados_en_valkimia", 
                                            dataframe=df_articulos_no_encontrados_en_valkimia)
            print("se creo el archivo articulos_no_encontrados_en_valkimia")
        else:
            print('no se creo el archivo articulos_no_encontrados_en_valkimia')
        ...
        print(f"df_ean_validacion_false = {df_ean_validacion_false.shape}")

        if (len(df_ean_validacion_false)>0):
            barcode_func.imprimir_dataframe(nombre_archivo="ean_validacion_false", 
                                            dataframe=df_ean_validacion_false)
            print("se creo el archivo ean_validacion_false")
        else:
            print('no se creo el archivo ean_validacion_false')
        ...
        print(f"df_ean_ocurrencias = {df_ean_ocurrencias.shape}")

        if (len(df_ean_ocurrencias)>0):
            barcode_func.imprimir_dataframe(nombre_archivo="ean_ocurrencias", 
                                            dataframe=df_ean_ocurrencias)
            print("se creo el archivo ean_ocurrencias")
        else:
            print('no se creo el archivo ean_ocurrencias')



    else:
        print("df_all sin registros")

    print(f"df_all = {df_all.shape}")
    barcode_func.imprimir_dataframe(nombre_archivo="df_all_procesado", dataframe=df_all)
    print("Se genera el df_all_procesado ")
